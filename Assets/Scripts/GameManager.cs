    ...
    public class GameManager : MonoBehaviour
    {
        ...
        /*void SubmitNewPosition()
        {
            if (NetworkManager.Singleton.IsServer && !NetworkManager.Singleton.IsClient)
            {
                ...
                {
                    var playerObject = NetworkManager.Singleton.SpawnManager.GetPlayerNetworkObject(uid);
                    var playerMovement = playerObject.GetComponent<PlayerMovement>(); // Get the PlayerMovement component
                    playerMovement.HandleMovement(); // Call HandleMovement instead of player.Move()
                }
            }
            else if (NetworkManager.Singleton.IsClient)
            {
